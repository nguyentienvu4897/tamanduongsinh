...
use App\Model\Admin\Ebrochure;
use App\Model\Admin\Showroom;
use Illuminate\Http\Request;
use App\Model\Admin\Ebrochure as ThisModel;
...
{
    protected $view = 'admin.eblogs';
    protected $route = 'e-brochure';

    public function index()
    {
        $showrooms = Ebrochure::query()->orderBy('sort')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $showroom = Ebrochure::query()->latest('sort')->first();
            $object->sort = $showroom ? ($showroom->sort + 1) : 1;

            $object->save();

            if ($request->image) {
                FileHelper::uploadFile($request->image, 'ebrochure', $object->id, ThisModel::class, 'image', 99);
            }

            if($request->file && $request->file != 'undefined') {
                $object->syncDocuments([$request->file], 'ebrochure/attachments/');
            }
    ...
    public function updateOrderShowroom(Request $request)
    {
        ...
        try {
            ...
            foreach ($showrooms as $index => $objId) {
                Ebrochure::query()->where('id', $objId)->update(['sort' => $index + 1]);
            }
